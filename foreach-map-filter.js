...
function vowelCount(str){
    const vowels = 'aeiou';
    const arrString = Array.from(str.toLowerCase());
    let finalObj = {};
    
    console.log(arrString);

    arrString.forEach(function(letter){
...
new array with each value multiplied by the index it is currently at in the array.
...
function removeVowels(str) {
    const arrStr = Array.from(str.toLowerCase());
    const vowels = 'aeiou';
    let newArr = [];
...
function doubleOddNumbers(arr) {
    let oddNums = [];
     oddNums = arr.filter(function(num){
        return num % 2 !==0;
    })
